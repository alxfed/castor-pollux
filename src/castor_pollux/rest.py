...
def continuation(contents, instruction, recorder=None, **kwargs):
    ...
    text = [
        {'parts': [{'text': 'In one sentence, who was Gerhard Gentzen?'}], 'role': 'user'}
    ]
...
if __name__ == '__main__':
    '''
    ['gemini-2.5-flash-preview-04-17', 'gemini-2.5-pro-exp-03-25', 'gemini-1.5-flash-latest','gemini-2.0-flash-lite','gemini-2.0-flash','gemini-2.0-pro-exp-02-05','gemini-2.0-flash-thinking-exp-01-21']
    '''
    instruction = {'parts': [{'text': 'You are an eloquent assistant'}], 'role': 'user'}
    text = [{
    'parts':[{'text': 'In one sentence, who was Gerhard Gentzen?'}]
    }]
    result = continuation(text, instruction)
    print(result)
